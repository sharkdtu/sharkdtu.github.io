...
针对该问题，业界其实也有一些讨论及方案，社区有一个重大特性提案 [FLIP-135](https://cwiki.apache.org/confluence/display/FLINK/FLIP-135+Approximate+Task-Local+Recovery) 就是尝试做有损的故障局部恢复，从2020年8月提出，到现在也没有太大进展，看社区规划并不打算继续推进。字节、快手内部自己实现了一种新的 Flink 故障恢复策略，当单个 Task 故障时，只重启失败的这个 Task，做到最小粒度的故障恢复，一定程度上可以保证作业故障期间不断流，但是通常情况下的抖动是伴随多个 Task 一起故障，那么就会连续触发多次故障恢复，整体恢复时间会很长，虽然不断流但是丢数据的时间较长，另外，他们的解决方案也未开源。
...
```dsconfig
# flink 自带的选项包括 region 和 full，我们新增了一项 taskmanager
jobmanager.execution.failover-strategy=taskmanager
# 为了应对同时多个 taskmanager 故障，使用 failure-rate 重启策略
restart-strategy=failure-rate
restart-strategy.failure-rate.delay=3s
# 一个周期内最多允许多少个 TaskManager 故障
restart-strategy.failure-rate.max-failures-per-interval=4
```

同样使用前面介绍的简单 MapReduce 示例测试，从 Pulsar 消费数据处理，使用 4 个 TaskManager。分别手动杀掉一个 TaskManager 和 两个 TaskManager 的数据消费曲线如下图所示，可以看到 TaskManager 异常时，不会断流（掉0），并在1分钟（JM与TM的心跳时间）左右快速恢复，这对于实时推荐类业务保持实时性是非常有效的。

<img src="/images/pulsar-consume.png" width="600" height="400" alt="pulsar-consume" align=center />

从我们一个实际业务情况来看，以往大概每次因为节点宕机、网络抖动等外部原因造成重启，有 10~15 分钟不等的数据掉 0 时间，应用我们提供的 TaskManager 故障恢复策略后，同样因为外部抖动，影响只是少量丢数据，并在 1 分钟左右就可以快速恢复，整体可用性大幅提升。

## 小结

本文介绍了微信 Flink 平台在适应实时推荐业务场景下实现的 TaskManager 故障恢复策略，目的是应对外部节点宕机、硬件故障、网络抖动等引发的 TaskManager 异常退出问题，做到异常时不断流，并快速恢复，保证实时性。除了“天灾”以外，顺带也可以解决一些“人祸”，例如用户业务逻辑里有内存泄漏，导致 TaskManager 运行一段时间因为 oom 被 kill 等问题。
